...
function ProductImage({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-purple-100 bg-white p-2 shadow-[0_24px_80px_rgba(124,58,237,0.22)] dark:border-purple-800/40 dark:bg-gray-950">
      ...
      <p className="type-label mt-3 px-2 pb-1 font-bold uppercase tracking-[0.16em] text-brand-600 dark:text-brand-300">
        {image.accent}
      </p>
    </div>
  )
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          >
            ...
            <p className="type-body mt-6 copy-body">
              At ChefKart, I work across customer-facing and internal products where clarity, usability and
              reliable frontend execution directly support customer actions and business operations.
            </p>
          </motion.div>
        </div>
        ...
        >
          <Tabs defaultValue={productAreas[0].id} className="w-full">
            <div className="overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <TabsList className="inline-flex h-auto min-w-max gap-3 bg-transparent p-0 copy-body">
            ...
            {productAreas.map(({ id, category, title, technologies, description, contributions, image }) => (
              <TabsContent key={id} value={id} className="mt-12">
                <article className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
                  <div className="order-2 lg:order-1">
                    <p className="type-label font-semibold uppercase tracking-[0.18em] text-accent1-600 dark:text-accent1-300">
                      {category}
                    </p>
                    <h3 className="type-section mt-3 copy-heading">{title}</h3>
                    <p className="type-body mt-5 max-w-2xl copy-body">{description}</p>

                    <div className="mt-8">
                      <p className="text-2xl font-bold copy-heading">Areas of contribution</p>
                      <ul className="mt-4 space-y-3">
                        {contributions.map((item) => (
                          <li
                            key={item}
                            className="flex gap-3 text-base leading-7 text-gray-700 dark:text-gray-300"
                          >
                  ...
                  <div className="relative order-1 min-h-[380px] lg:order-2 lg:min-h-[500px]">
                    ...
                    <div className="absolute -right-1 top-8 z-20 hidden rounded-full border border-purple-100 bg-white/90 px-4 py-2 shadow-lg shadow-purple-500/15 backdrop-blur dark:border-purple-800/40 dark:bg-gray-950/80 sm:block">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-bold uppercase tracking-[0.14em] copy-body">
                          UI focus
                        </span>
                      </div>
                    </div>
